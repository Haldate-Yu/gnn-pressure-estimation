...
def test_one_epoch(model: torch.nn.Module,
    ...
    with torch.no_grad():
        for data in loader:
            ...
            y_pred_rescaled = descale(scaled_data=y_pred, norm_type=norm_type, mean=mean, std=std, max=max_val,
                                      min=min_val)
            y_true_rescaled = descale(scaled_data=y_true, norm_type=norm_type, mean=mean, std=std, max=max_val,
                                      min=min_val)
...
def internal_train(args: argparse.Namespace,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tr_loss, tr_metric_dict, tr_record_metric_dict, out = train_one_epoch(model=model, optimizer=optimizer,
                                                                              loader=train_loader,
                                                                              # num_nodes=num_nodes,
                                                                              mask_rate=args.mask_rate, device=device,
                                                                              mean=mean, std=std, max_val=max_val,
                                                                              min_val=min_val, norm_type=args.norm_type,
                                                                              criterion=criterion,
        ...
        val_loss, val_metric_dict, val_record_metric_dict = test_one_epoch(model=model, loader=val_loader,
                                                                           # num_nodes=num_nodes,
                                                                           mask_rate=args.mask_rate, device=device,
                                                                           mean=mean, std=std, max_val=max_val,
                                                                           min_val=min_val, norm_type=args.norm_type,
        ...
        if epoch == 1 or (epoch % 5) == 0:
            print_metrics(epoch=epoch, tr_loss=tr_loss, val_loss=val_loss, tr_metric_dict=tr_record_metric_dict,
    ...
    trained_model, _ = load_checkpoint(path=os.path.join(args.save_path, f"best_{args.model_name}_{args.variant}.pth"),
                                       model=model)
...
def get_arguments(raw_args):
    ...
    parser.add_argument('--model', default='gatres_small', type=str,
                        choices=['gatres_small', 'gatres_large', 'gin', 'graphconvwat', 'chebnet', 'mgcn', 'gcn2',
                                 'gat'], help="support model selection only.")
    ...
    parser.add_argument('--test_data_path',
                        default=r"G:\.shortcut-targets-by-id\1uoKIPvTJgynIObYCS0Sktu0qX-U6wiSg\DiTEC\Data\Datasets[ size _ datatypes _ simtype _ notes _ name _ ddmmyyyy ]\0k288_hp_EPYNET_fullnodetime_Val-GEN-09 Oosterbeek_31032023.zip",
                        type=str, help="timed dataset path for testing")
    parser.add_argument('--test_input_path',
                        default=r"G:\.shortcut-targets-by-id\1uoKIPvTJgynIObYCS0Sktu0qX-U6wiSg\DiTEC\Data\Datasets[ size _ datatypes _ simtype _ notes _ name _ ddmmyyyy ]\WDN input files\Val-GEN-09Oosterbeek_20233101.inp",
                        type=str, help="timed input path for testing")
    ...
    parser.add_argument('--model_name', default=None, type=str,
                        help="Name of model. Keep its empty to use the name of class by default")
    parser.add_argument('--criterion', default='mse', choices=["mse", "mae", 'sce'], type=str,
                        help="criterion loss. Support mse|sce|mae")
    ...
    parser.add_argument('--use_data_edge_attrs', default=None, type=str,
                        help="pass pyg data edge attributes. Support: diameter| length| None")
    parser.add_argument('--patience', default=100, type=int,
                        help="Early stopping patience in these epochs. If val_loss unchanges, the training is stopped")
    parser.add_argument('--min_delta', default=1e-4, type=float, help="delta between last_loss and best_loss")
    parser.add_argument('--train_val_removal', default='keep_all',
                        choices=["keep_all", "keep_list", "keep_junction", "reservoir", "tank"], type=str,
    ...
    parser.add_argument('--use_gradient_clipping', default=False, type=bool,
                        help="Flag indicates gradient clipping is used in training")
    ...
    parser.add_argument('--scheduler', default=None, type=str, choices=['ReduceLROnPlateau', None],
    ...
    parser.add_argument('--scheduler_patience', default=2, type=int,
                        help="scheduler patience. Should be less than patience of early stopping")  #
    parser.add_argument('--norm_type', default='znorm', choices=["znorm", "minmax", "unused"], type=str,
                        help="normalization type. Support znorm| minmax|unused or None")
    ######TRACKING EXPERIMENTS SETTINGS######
    parser.add_argument('--log_method', default=None, choices=["wandb", None], type=str,
