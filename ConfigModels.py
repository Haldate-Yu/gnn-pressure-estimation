...
import argparse
import torch
from GraphModels import GraphConvWat, ChebNet, GATResMeanConv, m_GCN, GCN2, GAT, \
    GIN  # , GATResRewrittenConv, GATResLinearConv


def config_gatres_large(args: argparse.Namespace, test_model_variant_name=None) -> tuple[
    ...
    return args, GATResMeanConv(
        name='GATRes_Large_znorm_25b_128c' if test_model_variant_name is None else test_model_variant_name,
        num_blocks=25, nc=128)


def config_gatres_small(args: argparse.Namespace, test_model_variant_name=None) -> tuple[
    argparse.Namespace, torch.nn.Module]:
    ...
    args.norm_type = 'znorm'
    return args, GATResMeanConv(
        name='GATResMeanConv_small_znorm_15b_32c' if test_model_variant_name is None else test_model_variant_name,
        num_blocks=15, nc=32)
...
def config_mgcn(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    ...
    return args, m_GCN(name=test_model_variant_name, in_dim=1, out_dim=1, edge_dim=2, latent_dim=96, n_aggr=45,
                       n_hops=1, num_layers=2, batch_size=args.batch_size, w_sigmoid=False)


def config_graphconvwat(args: argparse.Namespace, test_model_variant_name=None) -> tuple[
    argparse.Namespace, torch.nn.Module]:
    ...
    args.use_data_edge_attrs = None
    args.norm_type = 'minmax'
    return args, GraphConvWat('GraphConvWat_minmax' if test_model_variant_name is None else test_model_variant_name, 1,
                              1)


def config_chebnet_graphconvwat_tuned(args: argparse.Namespace, test_model_variant_name=None) -> tuple[
    argparse.Namespace, torch.nn.Module]:
    args.model_path = r"experiments_logs\simple_test\chebnet_znorm\best_chebnet_znorm_20235722.pth"
    args.criterion = 'mse'
    args.use_data_edge_attrs = None
    args.norm_type = 'znorm'
    return args, ChebNet(name='ChebNet_znorm' if test_model_variant_name is None else test_model_variant_name,
                         in_channels=1, out_channels=1, nc=32)
...
def config_gat(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    ...
    args.use_data_edge_attrs = None
    args.norm_type = 'znorm'
    return args, GAT(name='GAT_10b_32c_2h' if test_model_variant_name is None else test_model_variant_name,
                     num_blocks=10, nc=32, in_channels=1, out_channels=1)
...
def config_gcn2(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    ...
    args.use_data_edge_attrs = None
    args.norm_type = 'znorm'
    return args, GCN2(name='GCN2-64b-32nc' if test_model_variant_name is None else test_model_variant_name,
                      num_blocks=64, nc=32, in_channels=1, out_channels=1)
...
def config_gin(args: argparse.Namespace, test_model_variant_name=None) -> tuple[argparse.Namespace, torch.nn.Module]:
    ...
    args.use_data_edge_attrs = None
    args.norm_type = 'znorm'
    return args, GIN(name='GIN_bottleneck_b15_c32' if test_model_variant_name is None else test_model_variant_name,
                     num_blocks=15, nc=32)
...
def config_gatres_small_tough(args: argparse.Namespace, test_model_variant_name=None) -> tuple[
    argparse.Namespace, torch.nn.Module]:
    ...
    args.norm_type = 'znorm'
    return args, GATResMeanConv(
        name='GATResMeanConv_small_tough_znorm_15b_32c' if test_model_variant_name is None else test_model_variant_name,
        num_blocks=15, nc=32)


def select_model(args: argparse.Namespace, test_model_variant_name: str = None, reset_model_path: bool = False) -> \
