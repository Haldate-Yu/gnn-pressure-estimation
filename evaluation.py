...
def test_one_epoch(model: torch.nn.Module,
    ...
    with torch.no_grad():
        for data in loader:
            ...
            wrapped_model = all_timer.auto_measure(model, num_graphs_per_batch=data.num_graphs,
                                                   gpu_warmup_times=gpu_warmup_times)
...
def internal_test(args: argparse.Namespace, model: torch.nn.Module, train_ds: WDNDataset, test_ds: WDNDataset,
                  do_load: bool = True) -> tuple[dict, dict, dict]:
    """perform a full test
    ...
    for k in test_sensor_metrics_dict.keys():
        ...
        out_test_sensor_metric_dict[f'{k}_mean'], out_test_sensor_metric_dict[f'{k}_std'] = torch.mean(x), torch.std(
            x) + 1e-6
...
def get_arguments(raw_args):
    ...
    parser.add_argument('--input_paths', default=[
        r"G:\.shortcut-targets-by-id\1uoKIPvTJgynIObYCS0Sktu0qX-U6wiSg\DiTEC\Data\Datasets[ size _ datatypes _ simtype _ notes _ name _ ddmmyyyy ]\WDN input files\GEN-09 Oosterbeek.inp"
        ], type=str, nargs='*', action='store',
    ...
    parser.add_argument('--test_data_path',
                        ...
                        type=str, help="timed dataset path for testing")  # 24hour
    ...
    parser.add_argument('--variant', default=datetime.now().strftime('%Y%M%d'), type=str,
                        help="Please give a value for model's variant")
    ...
    parser.add_argument('--train_val_removal', default='keep_all', type=str,
    ...
    parser.add_argument('--norm_type', default='znorm', type=str,
                        help="normalization type. Support znorm| minmax| unused")
    ...
    parser.add_argument('--test_type', default='clean', type=str, choices=['clean', 'noisy11', 'noisyNN'],
    ...
    parser.add_argument('--gpu_warmup_times', default=10, type=int,
                        help="Perform warmup inference N times before measuring latency and throughput")
    parser.add_argument('--use_same_mask', default=False, type=bool,
                        help="Flag indicates whether a single mask is applied to all snapshots in a test scenario. Default is False")
    parser.add_argument('--test_single_snapshot', default=False, type=bool,
