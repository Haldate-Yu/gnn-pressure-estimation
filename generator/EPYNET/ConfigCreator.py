...
parser.add_argument('--junc_demand_is_quantile', default=True, type=float,
                    help='flag indicates quantile. If True, dmd_lo and dmd_hi shoul be in range of [0,100]')
...
parser.add_argument('--junc_ele_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, junc_ele_lo and junc_ele_hi shoul be in range of [0,100]')
...
parser.add_argument('--pump_speed_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, pump_speed_lo and pump_speed_hi shoul be in range of [0,100]')
...
parser.add_argument('--pump_length_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, pump_length_lo and pump_length_hi shoul be in range of [0,100]')
...
parser.add_argument('--tank_level_is_quantile', default=True, type=float,
                    help='flag indicates quantile. If True, tank_level_lo and tank_level_hi shoul be in range of [0,100]')
...
parser.add_argument('--tank_ele_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, tank_ele_lo and tank_ele_hi shoul be in range of [0,100]')
...
parser.add_argument('--tank_dia_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, tank_dia_lo and tank_dia_hi shoul be in range of [0,100]')
...
parser.add_argument('--valve_dia_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, valve_dia_lo and valve_dia_hi shoul be in range of [0,100]')
...
parser.add_argument('--pipe_roughness_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, pipe_roughness_lo and pipe_roughness_hi shoul be in range of [0,100]')
...
parser.add_argument('--pipe_dia_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, pipe_dia_lo and pipe_dia_hi shoul be in range of [0,100]')
...
parser.add_argument('--pipe_length_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, pipe_length_lo and pipe_length_hi shoul be in range of [0,100]')
...
parser.add_argument('--pipe_minorloss_is_quantile', default=False, type=float,
                    help='flag indicates quantile. If True, pipe_minorloss_lo and pipe_minorloss_hi shoul be in range of [0,100]')
...
parser.add_argument('--head_add_ele', default=True, type=float,
                    help='total_head = ele_head + pressure_head. This flag allows the ele head addition')
...
def create_dummy_config(config_dir_path: str) -> tuple[ConfigParser, str]:
    ...
    junction_dict['demand_lo'], junction_dict['demand_hi'] = get_range(base_demands, args.junc_demand_lo,
                                                                       args.junc_demand_hi,
                                                                       args.junc_demand_is_quantile)

    elevations = wn.junctions.elevation.to_numpy()
    junction_dict['ele_lo'], junction_dict['ele_hi'] = get_range(elevations, args.junc_ele_lo, args.junc_ele_hi,
                                                                 args.junc_ele_is_quantile)
    ...
    if len(wn.pumps) > 0:
        ...
        pump_dict['speed_lo'], pump_dict['speed_hi'] = get_range(base_speeds, args.pump_speed_lo, args.pump_speed_hi,
                                                                 args.pump_speed_is_quantile)

        base_lengths = wn.pumps.length.to_numpy()
        pump_dict['length_lo'], pump_dict['length_hi'] = get_range(base_lengths, args.pump_length_lo,
                                                                   args.pump_length_hi, args.pump_length_is_quantile)
    ...
    if len(wn.tanks) > 0:
        ...
        tank_dict['ele_lo'], tank_dict['ele_hi'] = get_range(elevations, args.tank_ele_lo, args.tank_ele_hi,
                                                             args.tank_ele_is_quantile)

        diameters = wn.tanks.diameter.to_numpy()
        tank_dict['dia_lo'], tank_dict['dia_hi'] = get_range(diameters, args.tank_dia_lo, args.tank_dia_hi,
                                                             args.tank_dia_is_quantile)
    ...
    if len(wn.valves) > 0:
        ...
        valve_dict['dia_lo'], valve_dict['dia_hi'] = get_range(diameters, args.valve_dia_lo, args.valve_dia_hi,
                                                               args.valve_dia_is_quantile)
        config['valve'] = valve_dict
        ##############################################################################################
    pipe_dict = {}
    roughnesses = wn.pipes.roughness.to_numpy()
    pipe_dict['roughness_lo'], pipe_dict['roughness_hi'] = get_range(roughnesses, args.pipe_roughness_lo,
                                                                     args.pipe_roughness_hi,
                                                                     args.pipe_roughness_is_quantile)

    diameters = wn.pipes.diameter.to_numpy()
    pipe_dict['diameter_lo'], pipe_dict['diameter_hi'] = get_range(diameters, args.pipe_dia_lo, args.pipe_dia_hi,
                                                                   args.pipe_dia_is_quantile)

    lengths = wn.pipes.length.to_numpy()
    pipe_dict['length_lo'], pipe_dict['length_hi'] = get_range(lengths, args.pipe_length_lo, args.pipe_length_hi,
                                                               args.pipe_length_is_quantile)

    minorlosses = wn.pipes.minorloss.to_numpy()
    pipe_dict['minorloss_lo'], pipe_dict['minorloss_hi'] = get_range(minorlosses, args.pipe_minorloss_lo,
                                                                     args.pipe_minorloss_hi,
                                                                     args.pipe_minorloss_is_quantile)
