...
def get_keep_list(wn: WaterNetworkModel, removal: str, root: Union[zarr.Group, zarr.Array], feature: str) -> list[str]:
    if removal == 'keep_list':
        if root and "ordered_name_list" in root.attrs:
            ...
            print(
                f'WARN! ordered_name_list or ordered_names_by_attr attribute is not found in zarr file. Use removal = \'keep_junction\'')
...
class WDNDataset(Dataset):
    def __init__(self,
        ...
        assert removal in ['keep_list', 'reservoir', 'tank', 'keep_junction',
                           'keep_all'], f'Removal only supports keep_list,reservoir,tank,keep_junction. Got {removal}'
        _arrays = []
        _keeplists = []
        for i, (input_path, zip_file_path) in enumerate(zip(input_paths, zip_file_paths)):
            assert os.path.isfile(input_path) and (
                        input_path[-4:] == '.inp' or input_path[-4:] == '.net'), f'{input_path} is not a INP/ NET file'

            graph_template, array, keep_list = self.collect(input_path, zip_file_path, feature, edge_attrs, removal,
                                                            from_set, num_records, **kwargs)
        ...
        if do_scale:
            for i in range(len(self._arrays)):
                self._arrays[i] = scale(self._arrays[i], norm_type=norm_type, mean=self.mean, std=self.std,
            ...
            if edge_attrs:
                ...
                for i in range(len(self._templates)):
                    self._templates[i].edge_attr = scale(self._templates[i].edge_attr, norm_type=norm_type,
                                                         mean=self.edge_mean, std=self.edge_std, min=self.edge_min,
                                                         max=self.edge_max)
    ...
    def collect(self, input_path: str, zip_file_path: str, feature: str, edge_attrs: list, removal: str, from_set: str,
                num_records: int, **kwargs) -> tuple[Data, np.ndarray, list[str]]:
        ...
        assert set([feature]).issubset(
            root.group_keys()), f'feature {feature} is unavailabel in zarr file {zip_file_path}'
...
class NoisyWDNDataset(WDNDataset):
    ...
    def collect(self, input_path: str, zip_file_path: str, feature: str, edge_attrs: list, removal: str, from_set: str,
                num_records: int, **kwargs) -> tuple[zarr.Array, Data, np.ndarray, list[str]]:
        # addition params
